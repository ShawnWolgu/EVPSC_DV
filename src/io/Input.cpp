...
void skipLine(ifstream& file, int n) {
    if(n <= 0) return;
    string tp;
...
void readPaths(ifstream& ininp, string& outputStr) {
    string tp;

    getline(ininp, tp); // Check if the line is a number
    bool isNumber = true;
    for (char const &c : tp) {
        if (!isdigit(c)) {
            isNumber = false;
    ...
    if (isNumber) {
        // Multiple paths case
        int numPaths = stoi(tp);
        outputStr = to_string(numPaths);
...
// Helper function to check if line contains specific keyword
bool containsKeyword(const string& line, const string& keyword) {
    return line.find(keyword) != string::npos;
}

int EVPSCinput(string &ftex, string &fsx, string &fload, Procs::Process &Proc) {
    ifstream ininp("EVPSC_CPP.in");
    if (!ininp) {
    ...
    // Read texture paths
    getline(ininp, tp);
    if (containsKeyword(tp, "path") && containsKeyword(tp, "texture")) {
        readPaths(ininp, ftex);
    }
    
    // Read crystal paths
    getline(ininp, tp);
    if (containsKeyword(tp, "path") && containsKeyword(tp, "crystal")) {
        readPaths(ininp, fsx);
        
        // Validate texture and crystal configuration
        if (ftex[0] != fsx[0]) {
            logger.error("Error code 1: crystal configuration not match with texture file.");
            return EXIT_FAILURE;
        }
    }
    phaseCount = stoi(fsx.substr(0, fsx.find("(")));
    logger.info("Number of phases: " + to_string(phaseCount));
    
    // Read process paths
    getline(ininp, tp);
    if (containsKeyword(tp, "path") && containsKeyword(tp, "process")) {
        readPaths(ininp, fload);
    }
    processCount = stoi(fload.substr(0, fload.find("(")));
    logger.info("Number of processes: " + to_string(processCount));
...
int texinput(const string& fname, int& grainCount, vector<Vector4d>& eulerData){
    ifstream texinp(fname);
    if (!texinp){
    ...
    grainCount = static_cast<int>(Gn(0));
    if (grainCount <= 0) {
        logger.error("Error code 2: invalid grain count in texture file: " + std::to_string(grainCount));
        return 1;
    }
    
    eulerData.reserve(grainCount);
    for (int i = 0; i < grainCount; i++) {
        if (!getline(texinp, tp)) {
            logger.error("Error code 3: texture file format error - missing Euler angles for grain " + std::to_string(i));
            return 1;
        }
        try {
            Vector4d eulerAngles = getnum(tp, 4);
            eulerData.push_back(eulerAngles);
        }
        catch (const std::exception& e) {
            logger.error("Error code 4: failed to process Euler angles for grain " + std::to_string(i) + ": " + e.what());
            return 1;
        }
    }
    texinp.close(); 
...
string extractPath(const std::string& ftex, int a) {
    if (ftex.empty()) {
        throw std::invalid_argument("Error: Empty string in extractPath");
    }
    if (!isdigit(ftex[0])) {
    ...
    for (int i = 0; i < pathCount; ++i) {
        size_t openBracket = ftex.find('(', pos);
        if (openBracket == std::string::npos) {
            throw std::invalid_argument("Error: format error: missing left bracket in extractPath");
        }
        
        size_t closeBracket = ftex.find(')', openBracket);
        if (closeBracket == std::string::npos) {
            throw std::invalid_argument("Error: format error: missing right bracket in extractPath");
        }
        
        std::string path = ftex.substr(openBracket + 1, closeBracket - openBracket - 1);
        paths.push_back(path);
        
        pos = closeBracket + 1;
        
        if (i < pathCount - 1) {
            if (pos >= ftex.length() || ftex[pos] != ',') {
                throw std::invalid_argument("Error: format error: missing comma in extractPath");
            }
            pos++;
        }
    }
    return paths[a]; // 索引从1开始，所以要减1
}
